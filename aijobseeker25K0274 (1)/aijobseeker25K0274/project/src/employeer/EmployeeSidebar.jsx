...
import { Menu, X, Briefcase, Heart, User, Bell, LogOut, Search } from "lucide-react";
import PostJob from "./PostJob";
import JobApplications from "./JobApplications";
import EmployerProfileSettings from "./EmployerProfileSettings";
import EmployerAcceptedApplications from "./EmployerAcceptedApplications";
...
const { id } = useParams();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      >
        <div className="h-full px-3 pb-4 overflow-y-auto">
          <ul className="space-y-1 font-medium">
            {[ 
              { id: 1, name: "JobPost", icon: Briefcase, path: "/employeer/post-job" },
              { id: 2, name: "JobApplication", icon: Heart, path: "/employeer/job-application" },
              { id: 3, name: "Profile Settings", icon: User, path: "/employeer/profile-settings" },
              { id: 4, name: "Application", icon: Bell, path: "/employeer/accepted-applications" },
            ].map((item) => (
              <li key={item.id}>
                <NavLink
                  to={item.path}
      ...
      {id === "post-job" && <PostJob />}
      {id === "job-application" && <JobApplications />}
      {id === "profile-settings" && <EmployerProfileSettings />}
      {id === "accepted-applications" && <EmployerAcceptedApplications />}
      </main>
    </div>
  );
};
